...
// --- FIREBASE CONFIGURATION ---
        const firebaseConfig = {
            apiKey: "AIzaSyBr10KZfQ0v9SWKIEbuWzr3buCJtgEejoI",
            authDomain: "my-ledger-app-3a596.firebaseapp.com",
            databaseURL: "https://my-ledger-app-3a596-default-rtdb.asia-southeast1.firebasedatabase.app",
            projectId: "my-ledger-app-3a596",
            storageBucket: "my-ledger-app-3a596.firebasestorage.app",
            messagingSenderId: "780381890527",
            appId: "1:780381890527:web:5c3aa56777309ec5b1866a"
        };
        firebase.initializeApp(firebaseConfig);
        const auth = firebase.auth();
        const database = firebase.database();
        let currentUser = null;
        let savedExpensePaymentMethods = {};
        ...
        async function handleLogin() {
            ...
            if (!email || !password) { errorDiv.textContent = 'Email နှင့် Password ထည့်ပါ'; return; }
            try {
                await auth.signInWithEmailAndPassword(email, password);
            } catch (e) {
                errorDiv.textContent = e.message.includes('user-not-found') ? 'အကောင့်မတွေ့ပါ' :
                    e.message.includes('wrong-password') ? 'Password မှားနေပါသည်' :
                    e.message.includes('invalid-credential') ? 'Email သို့မဟုတ် Password မှားနေပါသည်' :
                    e.message.includes('invalid-email') ? 'Email ပုံစံမမှန်ပါ' : e.message;
            }
        }
        ...
        async function handleSignup() {
            ...
            errorDiv.textContent = '';
            if (!email || !password) { errorDiv.textContent = 'Email နှင့် Password ထည့်ပါ'; return; }
            if (password.length < 6) { errorDiv.textContent = 'Password အနည်းဆုံး ၆ လုံး ထည့်ပါ'; return; }
            if (password !== confirmPassword) { errorDiv.textContent = 'Password နှစ်ခု မတူညီပါ'; return; }
            try {
                await auth.createUserWithEmailAndPassword(email, password);
            } catch (e) {
                errorDiv.textContent = e.message.includes('email-already-in-use') ? 'ဤ Email ဖြင့် အကောင့်ရှိနေပြီးသားပါ' :
                    e.message.includes('invalid-email') ? 'Email ပုံစံမမှန်ပါ' : e.message;
            }
        }
        ...
        // --- FIREBASE REALTIME DATABASE BACKUP/RESTORE ---
        ...
        function backupToFirebase() {
            ...
            _backupTimer = setTimeout(function() {
                const backupData = {
                    transactions, moneyRecords, dailyOpeningBalances,
                    savedOtherTransactionTypes, savedOtherMoneyTypes,
                    tempCommissionData, debts, expenseEntries,
                    savedExpenseCategories, savedExpensePaymentMethods,
                    reminders, shownReminderToasts,
                    isScreenLockEnabled, appPIN: settingsPIN, permanentlyLocked,
                    lastUpdated: new Date().toISOString()
                };
                database.ref('users/' + currentUser.uid + '/appData').set(backupData)
                    .catch(err => console.error('Firebase backup error:', err));
            }, 2000);
        }

        // Also intercept all localStorage writes to trigger Firebase backup
        const _origSetItem = localStorage.setItem.bind(localStorage);
        localStorage.setItem = function(key, value) {
            _origSetItem(key, value);
            if (key.startsWith('moneyManager_')) backupToFirebase();
        };
            ...
            database.ref('users/' + currentUser.uid + '/appData').once('value')
                .then(snapshot => {
                    ...
                    if (data) {
                        ...
                        moneyRecords = Array.isArray(data.moneyRecords) ? data.moneyRecords : [];
                        dailyOpeningBalances = data.dailyOpeningBalances || {};
                        savedOtherTransactionTypes = data.savedOtherTransactionTypes || {};
                        savedOtherMoneyTypes = data.savedOtherMoneyTypes || {};
                        tempCommissionData = data.tempCommissionData || {};
                        debts = Array.isArray(data.debts) ? data.debts : [];
                        expenseEntries = Array.isArray(data.expenseEntries) ? data.expenseEntries : [];
                        savedExpenseCategories = data.savedExpenseCategories || {};
                        savedExpensePaymentMethods = data.savedExpensePaymentMethods || {};
                        reminders = Array.isArray(data.reminders) ? data.reminders : [];
                        shownReminderToasts = Array.isArray(data.shownReminderToasts) ? data.shownReminderToasts : [];
                        if (data.isScreenLockEnabled !== undefined) {
                            isScreenLockEnabled = data.isScreenLockEnabled;
                            localStorage.setItem('moneyManager_isScreenLockEnabled', isScreenLockEnabled);
                        }
                        if (data.appPIN) { settingsPIN = data.appPIN; localStorage.setItem('moneyManager_appPIN', settingsPIN); }
                        if (data.permanentlyLocked) { permanentlyLocked = data.permanentlyLocked; localStorage.setItem('moneyManager_permanentlyLocked', String(permanentlyLocked)); }
                        ...
                        try {
                            ...
                            localStorage.setItem('moneyManager_savedExpensePaymentMethods', JSON.stringify(savedExpensePaymentMethods));
                            localStorage.setItem('moneyManager_reminders', JSON.stringify(reminders));
                            localStorage.setItem('moneyManager_shownReminderToasts', JSON.stringify(shownReminderToasts));
                        ...
                        } catch(e) { console.error('Error caching to localStorage:', e); }
                    }
                    if (callback) callback();
                })
                .catch(err => { console.error('Firebase restore error:', err); if (callback) callback(); });
        }
        ...
        auth.onAuthStateChanged(function(user) {
            ...
            if (user) {
                ...
                // Restore from Firebase first, then initialize app
        ...
        const translations = {
            en: {
                ...
                profit: 'Total Profit',
                enter_transaction_id: 'Enter unique transaction ID',
                periodical_summaries: 'Periodical Summaries',
                debts_credits_chart: 'Debts/Credits Overview',
                daily: 'Daily',
                weekly: 'Weekly',
                monthly: 'Monthly',
                yearly: 'Yearly',
                reminders_alerts: 'Reminders & Alerts',
                reminder_description: 'Enter reminder description (e.g., Pay bill)',
                add_reminder: 'Add Reminder',
                reminder_due: 'Due',
                delete_reminder: 'Delete',
                debts_credits_history: 'Debts/Credits History',
                remove_custom_name: 'Remove Custom Name',
                no_payable_debts_found: 'No payable debts found.',
                no_receivable_credits_found: 'No receivable credits found.',
                delete_group: 'Delete Group',
                details: 'Details',
                permanent_lock_message: 'This application has been permanently locked due to too many failed attempts. All data has been wiped.',
                incorrect_pin: 'Incorrect PIN.',
                attempts_remaining: 'attempts remaining',
                incorrect_pin_try_again: 'Incorrect PIN. Please try again.',
                session_timed_out: 'Your session has timed out and the application has been locked.',
                fill_all_pin_fields: 'Please fill in all PIN fields.',
                new_pin_4_digits: 'New PIN must be exactly 4 digits.',
                pins_do_not_match: 'New PIN and Confirm New PIN do not match.',
                ...
                all_data_wiped: 'All application data has been wiped due to security measures.',
                local_storage_full: 'Local storage is full. Cannot save data.',
                error_saving_data: 'An error occurred while saving data.',
                error_reading_data: 'Error reading data from local storage',
                transaction_type_distribution_total: 'Transaction Type Distribution (Total Amount)',
                unknown_type: 'Unknown Type',
                total_amount: 'Total Amount',
                no_transaction_data: 'No Transaction Data',
                transaction_action_distribution_total: 'Transaction Action Distribution (Total Amount)',
                unknown_action: 'Unknown Action',
                daily_money_flow_actions_total: 'Daily Money Flow Actions (Total Amount)',
                no_money_flow_data: 'No Money Flow Data',
                daily_money_flow_types_total: 'Daily Money Flow Types (Total Amount)',
                expense_category_distribution_total: 'Expense Category Distribution (Total Amount)',
                unknown_category: 'Unknown Category',
                no_expense_data: 'No Expense Data',
                no_debts_data: 'No Debts Data',
                payable: 'Payable',
                receivable: 'Receivable',
                select_date_opening_balance: 'Please select a date for the opening balance.',
                opening_balances_saved_for: 'Opening balances saved for',
                error_saving_opening_balances: 'Error saving opening balances',
                backup_generated: 'Backup data generated!',
                confirm_restore_backup: 'This action will merge your existing data with the data from the text area. Continue?',
                no_backup_data: 'No backup data found in the text area',
                invalid_backup_format: 'Invalid backup data format (missing essential fields)',
                data_merged_success: 'Data merged successfully from text!',
                invalid_backup_or_error: 'Invalid backup data format or error during restore',
                backup_file_generated: 'Backup file generated!',
                invalid_file_type: 'Invalid file type. Please select a .json file.',
                confirm_merge_from_file: 'This action will merge your existing data with the data from',
                continue: 'Continue',
                invalid_backup_file_format: 'Invalid backup file format (missing essential fields)',
                data_merged_from_file: 'Data merged successfully from file!',
                error_parsing_backup: 'Error reading or parsing backup file',
                error_reading_file: 'Error reading file',
                specify_other_transaction_type: 'Please specify the other transaction type',
                ...
                commission_non_negative: 'Commission must be a non-negative number.',
                fee_non_negative: 'Fee must be a non-negative number.',
                transaction_id_exists: 'Transaction ID already exists. Please use a unique ID.',
                transaction_added_success: 'Transaction added successfully!',
                error_saving_transaction: 'Error saving transaction',
                edit_cancelled: 'Edit cancelled. Changes discarded.',
                edit_cancelled_error: 'Edit cancelled, but an error occurred restoring data.',
                error_loading_transaction: 'Error loading transaction for editing.',
                editing_transaction: 'Editing transaction. Click Add to save or Cancel to discard.',
                ...
                error_saving_after_deletion: 'Error saving changes after deletion',
                error_applying_date_filter: 'Error applying date filter.',
                no_custom_type_to_remove: 'No custom type entered to remove.',
                failed_remove_custom_type: 'Failed to remove custom type.',
                specify_other_money_type: 'Please specify the other money record type',
                type_required: 'Type is required.',
                money_record_added_success: 'Money record added successfully!',
                error_saving_money_record: 'Error saving money record',
                no_money_records_found: 'No money records found.',
                error_loading_money_record: 'Error loading money record for editing.',
                money_record_loaded_for_editing: 'Money record loaded for editing',
                confirm_delete_record: 'Are you sure you want to delete this record?',
                money_record_deleted_success: 'Money record deleted successfully',
                error_deleting_money_record: 'Error deleting money record.',
                specify_other_category: 'Please specify the other category',
                specify_other_method: 'Please specify the other payment method',
                category_required: 'Category is required.',
                payment_method_required: 'Payment Method is required.',
                expense_income_added_success: 'Expense/Income entry added successfully!',
                error_saving_expense_entry: 'Error saving expense entry',
                no_expense_income_records: 'No expense/income records found.',
                error_loading_expense_entry: 'Error loading expense entry for editing.',
                expense_entry_loaded_for_editing: 'Expense entry loaded for editing',
                confirm_delete_entry: 'Are you sure you want to delete this entry?',
                entry_deleted_success: 'Entry deleted successfully',
                error_deleting_expense_entry: 'Error deleting expense entry.',
                no_custom_category_to_remove: 'No custom category entered to remove.',
                failed_remove_custom_category: 'Failed to remove custom category.',
                no_custom_method_to_remove: 'No custom payment method entered to remove.',
                failed_remove_custom_method: 'Failed to remove custom payment method.',
                name_required: 'Name is required.',
                debt_credit_updated_success: 'Debt/Credit entry updated successfully!',
                debt_credit_added_success: 'Debt/Credit added successfully!',
                update_entry: 'Update Entry',
                no_entry_edited: 'No entry was being edited.',
                no_debts_credits_found: 'No debts/credits found.',
                initial_debt: 'Initial Debt',
                initial_credit: 'Initial Credit',
                paid: 'Paid',
                received: 'Received',
                hide_details: 'Hide Details',
                entry_loaded_for_editing: 'Entry loaded for editing. Update form and click Update.',
                entry_not_found_editing: 'Entry not found for editing.',
                confirm_delete_history_entry: 'Are you sure you want to delete this history entry?',
                history_entry_deleted_success: 'History entry deleted successfully',
                entry_not_found_deletion: 'Entry not found for deletion.',
                confirm_delete_group: 'Are you sure you want to delete the entire group for',
                action_cannot_undone: 'This action cannot be undone.',
                debt_group_deleted_success: 'Debt group deleted successfully',
                group_not_found_deletion: 'Group not found for deletion.',
                no_custom_name_to_remove: 'No custom name entered to remove.',
                reminder_fields_required: 'Description and due date are required.',
            ...
            my: {
                ...
                lock_screen_message: 'လုပ်ဆောင်မှုမရှိခြင်းကြောင့် သင့်အက်ပပလီကေးရှင်းကို လော့ချထားပါသည်။',
                ...
                total_net_amount: 'စုစုပေါင်း လက်ကျန်ပမာဏ:',
                ...
                add_new_expense_income_entry: 'အသုံးစရိတ်/ဝင်ငွေ မှတ်တမ်း',
                expense: 'အသုံးစရိတ်',
                income: 'ဝင်ငွေ',
                category: 'အမျိုးအစား',
                rent: 'ငှားရမ်းခ',
                ...
                optional_expense_note: 'ရွေးချယ်ရန်: အသုံးစရိတ်ရည်ရွယ်ချက်၊ ဝင်ငွေ ရင်းမြစ်',
                ...
                add_new_debt_credit_entry: 'ကြွေးမြီ/ချေးငွေ မှတ်တမ်း အသစ်',
                name: 'အမည်',
                ...
                enter_current_pin: 'လက်ရှိ PIN ထည့်ပါ',
                enter_new_pin: 'ဂဏန်း ၄ လုံးပါ PIN အသစ်ထည့်ပါ',
                total_balance: 'စုစုပေါင်း လက်ကျန်',
                profit: 'စုစုပေါင်း အမြတ်',
                enter_transaction_id: 'ထူးခြားသော ငွေလွှဲမှု ID ထည့်ပါ',
                periodical_summaries: 'ကာလအလိုက် အကျဉ်းချုပ်များ',
                debts_credits_chart: 'ကြွေးမြီ/ချေးငွေ အကျဉ်းချုပ်',
                daily: 'နေ့စဉ်',
                weekly: 'အပတ်စဉ်',
                monthly: 'လစဉ်',
                yearly: 'နှစ်စဉ်',
                reminders_alerts: 'သတိပေးချက်များနှင့် အချက်ပြမှု',
                reminder_description: 'သတိပေးချက်ကို ထည့်ပါ (ဥပမာ၊ ဘေဆောင်ပါ)',
                add_reminder: 'သတိပေးချက်ထည့်ပါ',
                reminder_due: 'ကျရောက်မည်',
                delete_reminder: 'ဖျက်ပါ',
                debts_credits_history: 'ကြွေးမြီ/ချေးငွေ မှတ်တမ်း',
                remove_custom_name: 'ကိုယ်တိုင် အမည်ကို ဖယ်ရှားပါ',
                no_payable_debts_found: 'ပေးချေရမည့် ကြွေးမြီများ မတွေ့ပါ။',
                no_receivable_credits_found: 'လက်ခံရမည့် ချေးငွေများ မတွေ့ပါ။',
                delete_group: 'အဖွဲ့ ဖျက်ပါ',
                details: 'အသေးစိတ်',
                permanent_lock_message: 'မှားယွင်းသော ကြိုးစားမှု များပြားခြင်းကြောင့် ဤအက်ပလီကေးရှင်းကို အပြီးတိုင် လော့ချထားပါသည်။ ဒေတာ အားလုံး ဖျက်ပစ်ပြီး။',
                incorrect_pin: 'မှားယွင်းသော PIN။',
                attempts_remaining: 'ကြိုးစားမှု ကျန်ရှိပါသည်',
                incorrect_pin_try_again: 'မှားယွင်းသော PIN။ ထပ်မံကြိုးစားပါ။',
                session_timed_out: 'သင့်ဆက်ရှင်က အချိန်ကုန်သွားပြီ ဖြစ်ပြီး အက်ပလီကေးရှင်းကို လော့ချထားပါသည်။',
                fill_all_pin_fields: 'PIN အကွက်များအားလုံးကို ဖြည့်ပါ။',
                new_pin_4_digits: 'PIN အသစ်သည် အတိအကျ ၄ ဂဏန်း ရှိရမည်။',
                pins_do_not_match: 'PIN အသစ်နှင့် အတည်ပြု PIN အသစ်တို့ မကိုက်ညီပါ။',
                incorrect_current_pin: 'မှားယွင်းသော လက်ရှိ PIN။',
                pin_changed_success: 'PIN ကို အောင်မြင်စွာ ပြောင်းလဲပြီး!',
                dark_mode_enabled: 'အမှောင်မုဒ် ဖွင့်ထားပါသည်။',
                light_mode_enabled: 'အလင်းမုဒ် ဖွင့်ထားပါသည်။',
                language_switched: 'ဘာသာစကားပြောင်းလဲပြီး။',
                all_data_wiped: 'လုံခြုံရေး အစီအမံကြောင့် အက်ပလီကေးရှင်း ဒေတာ အားလုံး ဖျက်ပစ်ပြီး။',
                local_storage_full: 'လိုကယ်စတိုးရှင်း ပြည့်နေပါသည်။ ဒေတာ မသိမ်းဆည်းနိုင်ပါ။',
                error_saving_data: 'ဒေတာ သိမ်းဆည်းရာတွင် အမှားတစ်ခု ဖြစ်ပွားပါသည်။',
                error_reading_data: 'လိုကယ်စတိုးရှင်းမှ ဒေတာ ဖတ်ရာတွင် အမှားဖြစ်ပွားပါသည်',
                transaction_type_distribution_total: 'ငွေလွှဲမှုအမျိုးအစား ဖြန့်ဖြူးမှု (စုစုပေါင်း ပမာဏ)',
                unknown_type: 'မသိရှိသော အမျိုးအစား',
                total_amount: 'စုစုပေါင်း ပမာဏ',
                no_transaction_data: 'ငွေလွှဲမှု ဒေတာ မရှိပါ',
                transaction_action_distribution_total: 'ငွေလွှဲမှုလုပ်ဆောင်ချက် ဖြန့်ဖြူးမှု (စုစုပေါင်း ပမာဏ)',
                unknown_action: 'မသိရှိသော လုပ်ဆောင်ချက်',
                daily_money_flow_actions_total: 'နေ့စဉ်ငွေစီးဆင်းမှုလုပ်ဆောင်ချက်များ (စုစုပေါင်း ပမာဏ)',
                no_money_flow_data: 'ငွေစီးဆင်းမှု ဒေတာ မရှိပါ',
                daily_money_flow_types_total: 'နေ့စဉ်ငွေစီးဆင်းမှုအမျိုးအစားများ (စုစုပေါင်း ပမာဏ)',
                expense_category_distribution_total: 'အသုံးစရိတ်အမျိုးအစား ဖြန့်ဖြူးမှု (စုစုပေါင်း ပမာဏ)',
                unknown_category: 'မသိရှိသော အမျိုးအစား',
                no_expense_data: 'အသုံးစရိတ် ဒေတာ မရှိပါ',
                no_debts_data: 'ကြွေးမြီ ဒေတာ မရှိပါ',
                payable: 'ပေးချေရမည်',
                receivable: 'လက်ခံရမည်',
                select_date_opening_balance: 'ဖွင့်လှစ်ချင်း လက်ကျန်အတွက် နေ့စွဲကို ရွေးပါ။',
                opening_balances_saved_for: 'ဖွင့်လှစ်ချင်း လက်ကျန်များကို သိမ်းဆည်းပြီး',
                error_saving_opening_balances: 'ဖွင့်လှစ်ချင်း လက်ကျန်များကို သိမ်းဆည်းရာတွင် အမှားဖြစ်ပွားပါသည်',
                backup_generated: 'အရန်ကူးယူ ဒေတာ ဖန်တီးပြီး!',
                confirm_restore_backup: 'ဤလုပ်ဆောင်ချက်သည် သင့်ရှိပြီးသားဒေတာကို စာသားဧရိယာမှ ဒေတာနှင့် ပေါင်းစည်းမည်။ ဆက်လုပ်ပါမည်လား?',
                no_backup_data: 'စာသားဧရိယာတွင် အရန်ကူးယူ ဒေတာ မတွေ့ပါ',
                invalid_backup_format: 'မမှန်ကန်သော အရန်ကူးယူ ဒေတာ ပုံစံ (အဓိက အကွက်များ မရှိပါ)',
                data_merged_success: 'စာသားမှ ဒေတာ အောင်မြင်စွာ ပေါင်းစည်းပြီး!',
                invalid_backup_or_error: 'မမှန်ကန်သော အရန်ကူးယူ ဒေတာ ပုံစံ သို့မဟုတ် ပြန်လည်ရယူရာတွင် အမှားဖြစ်ပွားပါသည်',
                backup_file_generated: 'အရန်ကူးယူ ဖိုင် ဖန်တီးပြီး!',
                invalid_file_type: 'မမှန်ကန်သော ဖိုင်အမျိုးအစား။ .json ဖိုင်ကို ရွေးပါ။',
                confirm_merge_from_file: 'ဤလုပ်ဆောင်ချက်သည် သင့်ရှိပြီးသားဒေတာကို ဖိုင်မှ ဒေတာနှင့် ပေါင်းစည်းမည်။ ဆက်လုပ်ပါမည်လား?',
                continue: 'ဆက်လက်ပါ',
                invalid_backup_file_format: 'မမှန်ကန်သော အရန်ကူးယူ ဖိုင်ပုံစံ (အဓိက အကွက်များ မရှိပါ)',
                data_merged_from_file: 'ဖိုင်မှ ဒေတာ အောင်မြင်စွာ ပေါင်းစည်းပြီး!',
                error_parsing_backup: 'အရန်ကူးယူ ဖိုင်ကို ဖတ်ရန် သို့မဟုတ် ခွဲခြမ်းရန် အမှားဖြစ်ပွားပါသည်',
                error_reading_file: 'ဖိုင်ဖတ်ရန် အမှားဖြစ်ပွားပါသည်',
                specify_other_transaction_type: 'အခြားငွေလွှဲမှုအမျိုးအစားကို သတ်မှတ်ပါ',
                transaction_id_required: 'ငွေလွှဲမှု ID လိုအပ်ပါသည်။',
                date_time_required: 'နေ့စွဲ/အချိန် လိုအပ်ပါသည်။',
                transaction_type_required: 'ငွေလွှဲမှုအမျိုးအစား လိုအပ်ပါသည်။',
                action_required: 'လုပ်ဆောင်ချက် လိုအပ်ပါသည်။',
                amount_positive: 'ပမာဏသည် အပေါင်းဂဏန်း ဖြစ်ရမည်။',
                commission_non_negative: 'ကော်မရှင်သည် အပျက်မဟုတ်သော ဂဏန်း ဖြစ်ရမည်။',
                fee_non_negative: 'ကောက်ခံသည် အပျက်မဟုတ်သော ဂဏန်း ဖြစ်ရမည်။',
                transaction_id_exists: 'ငွေလွှဲမှု ID ရှိနေပါပြီ။ ထူးခြားသော ID ကို အသုံးပြုပါ။',
                transaction_added_success: 'ငွေလွှဲမှု အောင်မြင်စွာ ထည့်ပြီး!',
                error_saving_transaction: 'ငွေလွှဲမှုကို သိမ်းဆည်းရာတွင် အမှားဖြစ်ပွားပါသည်',
                edit_cancelled: 'တည်းဖြတ်မှု ပယ်ဖျက်ပယ်ဖျက်ပြီး။ ပြောင်းလဲမှုများကို မလုပ်တော့ပါ။',
                edit_cancelled_error: 'တည်းဖြတ်မှု ပယ်ဖျက်ပြီး၊ ဒေတာ ပြန်လည်ရယူရာတွင် အမှားဖြစ်ပွားပါသည်။',
                error_loading_transaction: 'ငွေလွှဲမှုကို တည်းဖြတ်ရန် ဖွင့်ရာတွင် အမှားဖြစ်ပွားပါသည်။',
                editing_transaction: 'ငွေလွှဲမှုကို တည်းဖြတ်နေပါသည်။ သိမ်းရန် ထည့်ပါ ကို နှိပ်ပါ သို့မဟုတ် ပယ်ဖျက်ရန် ပယ်ဖျက်ပါ ကို နှိပ်ပါ။',
                confirm_delete_transaction: 'ဤငွေလွှဲမှုကို ဖျက်ရန် သေချာပါသလား?',
                transaction_deleted_success: 'ငွေလွှဲမှု ဖျက်ပြီး အောင်မြင်ပါသည်',
                error_deleting_transaction: 'ငွေလွှဲမှုကို ဖျက်ရန် အမှားဖြစ်ပွားပါသည်။',
                error_saving_after_deletion: 'ဖျက်ပြီးနောက် ပြောင်းလဲမှုများကို ပြန်လည်သိမ်းဆည်းရာတွင် အမှားဖြစ်ပွားပါသည်',
                error_applying_date_filter: 'နေ့စွဲ စစ်ထုတ်မှု အသုံးပြုရာတွင် အမှားဖြစ်ပွားပါသည်။',
                no_custom_type_to_remove: 'ဖယ်ရှားရန် ကိုယ်တိုင်အမျိုးအစားကို မထည့်ရသေးပါ။',
                failed_remove_custom_type: 'ကိုယ်တိုင်အမျိုးအစားကို ဖယ်ရှားရန် မအောင်မြင်ပါ။',
                specify_other_money_type: 'အခြားငွေ မှတ်တမ်း အမျိုးအစားကို သတ်မှတ်ပါ',
                type_required: 'အမျိုးအစား လိုအပ်ပါသည်။',
                money_record_added_success: 'ငွေ မှတ်တမ်း အောင်မြင်စွာ ထည့်ပြီး!',
                error_saving_money_record: 'ငွေ မှတ်တမ်းကို သိမ်းဆည်းရာတွင် အမှားဖြစ်ပွားပါသည်',
                no_money_records_found: 'ငွေ မှတ်တမ်း မတွေ့ပါ။',
                error_loading_money_record: 'ငွေ မှတ်တမ်း တည်းဖြတ်ရန် ဖွင့်ရာတွင် အမှားဖြစ်ပွားပါသည်။',
                money_record_loaded_for_editing: 'ငွေ မှတ်တမ်း တည်းဖြတ်ရန် ဖွင့်ပြီး',
                confirm_delete_record: 'ဤမှတ်တမ်းကို ဖျက်ရန် သေချာပါသလား?',
                money_record_deleted_success: 'ငွေ မှတ်တမ်း ဖျက်ပြီး အောင်မြင်ပါသည်',
                error_deleting_money_record: 'ငွေ မှတ်တမ်း ဖျက်ရန် အမှားဖြစ်ပွားပါသည်။',
                specify_other_category: 'အခြားအမျိုးအစားကို သတ်မှတ်ပါ',
                specify_other_method: 'အခြားပေးချေမှုနည်းလမ်းကို သတ်မှတ်ပါ',
                category_required: 'အမျိုးအစား လိုအပ်ပါသည်။',
                payment_method_required: 'ပေးချေမှုနည်းလမ်း လိုအပ်ပါသည်။',
                expense_income_added_success: 'အသုံးစရိတ်/ဝင်ငွေ မှတ်တမ်း အောင်မြင်စွာ ထည့်ပြီး!',
                error_saving_expense_entry: 'အသုံးစရိတ် မှတ်တမ်းကို သိမ်းဆည်းရာတွင် အမှားဖြစ်ပွားပါသည်',
                no_expense_income_records: 'အသုံးစရိတ်/ဝင်ငွေ မှတ်တမ်းများ မတွေ့ပါ။',
                error_loading_expense_entry: 'အသုံးစရိတ် မှတ်တမ်းကို တည်းဖြတ်ရန် ဖွင့်ရာတွင် အမှားဖြစ်ပွားပါသည်။',
                expense_entry_loaded_for_editing: 'အသုံးစရိတ် မှတ်တမ်း တည်းဖြတ်ရန် ဖွင့်ပြီး',
                confirm_delete_entry: 'ဤမှတ်တမ်း ကို ဖျက်ရန် သေချာပါသလား?',
                entry_deleted_success: 'မှတ်တမ်း ဖျက်ပြီး အောင်မြင်ပါသည်',
                error_deleting_expense_entry: 'အသုံးစရိတ် မှတ်တမ်း ဖျက်ရန် အမှားဖြစ်ပွားပါသည်။',
                no_custom_category_to_remove: 'ဖယ်ရှားရန် ကိုယ်တိုင်အမျိုးအစားကို မထည့်ရသေးပါ။',
                failed_remove_custom_category: 'ကိုယ်တိုင်အမျိုးအစားကို ဖယ်ရှားရန် မအောင်မြင်ပါ။',
                no_custom_method_to_remove: 'ဖယ်ရှားရန် ကိုယ်တိုင်ပေးချေမှုနည်းလမ်းကို မထည့်ရသေးပါ။',
                failed_remove_custom_method: 'ကိုယ်တိုင်ပေးချေမှုနည်းလမ်းကို ဖယ်ရှားရန် မအောင်မြင်ပါ။',
                name_required: 'အမည် လိုအပ်ပါသည်။',
                debt_credit_updated_success: 'ကြွေးမြီ/ချေးငွေ မှတ်တမ်း အောင်မြင်စွာ အပ်ဒိတ်ပြီး!',
                debt_credit_added_success: 'ကြွေးမြီ/ချေးငွေ အောင်မြင်စွာ ထည့်ပြီး!',
                update_entry: 'မှတ်တမ်း အပ်ဒိတ်ပါ',
                no_entry_edited: 'တည်းဖြတ်နေသော မှတ်တမ်း မရှိပါ။',
                no_debts_credits_found: 'ကြွေးမြီ/ချေးငွေ မတွေ့ပါ။',
                initial_debt: 'အစ ကြွေးမြီ',
                initial_credit: 'အစ ချေးငွေ',
                paid: 'ပေးချေပြီး',
                received: 'ရရှိပြီး',
                hide_details: 'အသေးစိတ်ကို ဖုံးကွယ်ပါ',
                entry_loaded_for_editing: 'မှတ်တမ်း တည်းဖြတ်ရန် ဖွင့်ပြီး။ ဖောင်ကို အပ်ဒိတ်ပါ နှင့် အပ်ဒိတ်ကို နှိပ်ပါ။',
                entry_not_found_editing: 'တည်းဖြတ်ရန် မှတ်တမ်း မတွေ့ပါ။',
                confirm_delete_history_entry: 'ဤမှတ်တမ်း မှတ်တမ်း ကို ဖျက်ရန် သေချာပါသလား?',
                history_entry_deleted_success: 'မှတ်တမ်း ဖျက်ပြီး အောင်မြင်ပါသည်',
                entry_not_found_deletion: 'ဖျက်ရန် မှတ်တမ်း မတွေ့ပါ။',
                confirm_delete_group: 'အဖွဲ့ တစ်ခုလုံးကို ဖျက်ရန် သေချာပါသလား',
                action_cannot_undone: 'ဤလုပ်ဆောင်ချက်ကို မပြန်လည်နိုင်ပါ။',
                debt_group_deleted_success: 'ကြွေးမြီ အဖွဲ့ ဖျက်ပြီး အောင်မြင်ပါသည်',
                group_not_found_deletion: 'ဖျက်ရန် အဖွဲ့ မတွေ့ပါ။',
                no_custom_name_to_remove: 'ဖယ်ရှားရန် ကိုယ်တိုင်အမည်ကို မထည့်ရသေးပါ။',
                reminder_fields_required: 'ဖော်ပြချက်နှင့် ကျရောက်မည့်ရက်စွဲ လိုအပ်ပါသည်။',
                reminder_added_success: 'သတိပေးချက် အောင်မြင်စွာ ထည့်ပြီး!',
                reminder_deleted_success: 'သတိပေးချက် ဖျက်ပြီး အောင်မြင်ပါသည်',
                no_reminders_found: 'သတိပေးချက်များ မတွေ့ပါ။',
                confirm_delete_reminder: 'ဤသတိပေးချက်ကို ဖျက်ရန် သေချာပါသလား?',
                reminder_due: 'သတိပေးချက် ကျရောက်မည်',
                method: 'နည်းလမ်း:'
            }
        };
        ...
        let debtsCreditsChart = null; // New chart for debts/credits
        ...
        // Reminders array
        let reminders = []; // New array for reminders
        let shownReminderToasts = JSON.parse(localStorage.getItem('moneyManager_shownReminderToasts')) || []; // To track shown reminders
        ...
        function wipeAllData() {
            const keysToRemove = [
                ...
                'moneyManager_language', 'moneyManager_reminders', 'moneyManager_shownReminderToasts' // Add shown toasts key
            ];
            ...
            shownReminderToasts = []; // Reset shown toasts
        ...
        function showToast(message, type = 'success') {
            ...
            const duration = type === 'warning' ? 10000 : 3000;
            setTimeout(() => {
        ...
        function addOptionToSelect(selectId, optionValue) {
            ...
            if (!optionExists && normalizedOptionValue && normalizedOptionValue !== 'other') {
                ...
                newOption.value = optionValue.trim(); // Keep original casing for value
        ...
        function saveData() {
            try {
                ...
                localStorage.setItem('moneyManager_reminders', JSON.stringify(reminders)); // Save reminders
                localStorage.setItem('moneyManager_shownReminderToasts', JSON.stringify(shownReminderToasts)); // Save shown toasts
            } catch (e) {
        ...
        function loadData() {
            ...
            const savedReminders = localStorage.getItem('moneyManager_reminders'); // Load reminders
            const savedShownToasts = localStorage.getItem('moneyManager_shownReminderToasts'); // Load shown toasts
            ...
            try {
                ...
                reminders = savedReminders ? JSON.parse(savedReminders) : []; // Parse reminders
                shownReminderToasts = savedShownToasts ? JSON.parse(savedShownToasts) : []; // Parse shown toasts
            ...
            if (!Array.isArray(reminders)) reminders = []; // Validate reminders
            if (!Array.isArray(shownReminderToasts)) shownReminderToasts = []; // Validate shown toasts

            updateExistingDebtNames();
            populateCustomTypes();
            populateExpenseCategories();
            populatePaymentMethods();
            populateDebtNameSelect(); // New: populate debt name select
        }
        ...
        function populateCustomTypes() {
            ...
            for (let type in savedOtherTransactionTypes) {
                if (savedOtherTransactionTypes.hasOwnProperty(type) && type.trim() !== '' && type.toLowerCase().trim() !== 'other' && !existingTransactionTypes.includes(type.toLowerCase().trim())) {
            ...
            for (let type in savedOtherMoneyTypes) {
                if (savedOtherMoneyTypes.hasOwnProperty(type) && type.trim() !== '' && type.toLowerCase().trim() !== 'other' && !existingMoneyTypes.includes(type.toLowerCase().trim())) {
        ...
        function populateExpenseCategories() {
            ...
            for (let category in savedExpenseCategories) {
                if (savedExpenseCategories.hasOwnProperty(category) && category.trim() !== '' && category.toLowerCase().trim() !== 'other' && !existingCategories.includes(category.toLowerCase().trim())) {
        ...
        function populatePaymentMethods() {
            ...
            for (let method in savedExpensePaymentMethods) {
                if (savedExpensePaymentMethods.hasOwnProperty(method) && method.trim() !== '' && method.toLowerCase().trim() !== 'other' && !existingMethods.includes(method.toLowerCase().trim())) {
                    addOptionToSelect('expense-payment-method', method);
                }
            }
        }

        // New: Populates the debt name select dropdown with existing names
        function populateDebtNameSelect() {
            const debtNameSelect = document.getElementById('debt-name');
            debtNameSelect.innerHTML = ''; // Clear existing options except 'other'

            // Add existing names as options
            existingDebtNames.sort().forEach(name => {
                addOptionToSelect('debt-name', name);
            });

            // Add 'other' option at the end
            const otherOption = document.createElement('option');
            otherOption.value = 'other';
            otherOption.textContent = translations[currentLanguage]['other'];
            debtNameSelect.appendChild(otherOption);
        }
        ...
        function destroyAllCharts() {
            ...
            if (debtsCreditsChart) debtsCreditsChart.destroy(); // Destroy debts chart
            ...
            debtsCreditsChart = null; // Reset debts chart
        }
        ...
        function updateChartColors() {
            ...
            if (debtsCreditsChart) {
                debtsCreditsChart.options.plugins.title.color = chartTextColor;
                debtsCreditsChart.options.plugins.legend.labels.color = chartTextColor;
                debtsCreditsChart.options.scales.x.ticks.color = chartTextColor;
                debtsCreditsChart.options.scales.y.ticks.color = chartTextColor;
                debtsCreditsChart.update();
            }
        }
        ...
        function updateChartsWithFilter() {
            const period = document.getElementById('charts-period-select').value;

            // Filter data for each chart
            const filteredTransactions = getFilteredData(transactions, period);
            const filteredMoneyRecords = getFilteredData(moneyRecords, period);
            const filteredExpenseEntries = getFilteredData(expenseEntries, period);

            // For debts, filter history within each group
            const filteredDebts = debts.map(group => ({
                ...group,
                history: getFilteredData(group.history, period)
            })).filter(group => group.history.length > 0);

            // Redraw charts with filtered data
            drawTransactionCharts(filteredTransactions);
            drawDailyMoneyCharts(filteredMoneyRecords);
            drawExpenseCharts(filteredExpenseEntries);
            drawDebtsCreditsChart(filteredDebts);
        }

        // Autoredraw charts when the page loads or the user navigates to the tab
        window.addEventListener('DOMContentLoaded', () => {
            updateChartsWithFilter();
        });

        function drawTransactionCharts(filteredData = transactions) {
            // Transaction Type Distribution (Total Amount)
            const transactionTypes = {};
            const currentTransactions = Array.isArray(filteredData) ? filteredData : []; // Ensure it's an array
            ...
            transactionTypeChart = new Chart(transactionTypeCtx, {
                ...
                options: {
                    ...
                    plugins: {
                        ...
                        legend: { display: true, labels: { color: isDarkMode ? 'white' : '#495057' } },
                    },
                    scales: {
                        x: { beginAtZero: true, ticks: { color: isDarkMode ? 'white' : '#495057' } },
                        y: { ticks: { color: isDarkMode ? 'white' : '#495057' } }
                    }
                }
            });
        ...
        function drawDailyMoneyCharts(filteredData = moneyRecords) {
            // Daily Money Flow Actions (Total Amount)
            const moneyActions = {};
            const currentMoneyRecords = Array.isArray(filteredData) ? filteredData : [];
        ...
        function drawExpenseCharts(filteredData = expenseEntries) {
            // Expense Category Distribution (Total Amount)
            const expenseCategoriesData = {};
            const currentExpenseEntries = Array.isArray(filteredData) ? filteredData : [];
        ...
        function drawDebtsCreditsChart(filteredDebts = debts) {
            const names = [];
            let payableAmounts = [];
            let receivableAmounts = [];
            filteredDebts.forEach(group => {
                if (!names.includes(group.name)) names.push(group.name);
            });
            names.forEach(name => {
                let payable = 0;
                let receivable = 0;
                filteredDebts.filter(group => group.name === name).forEach(group => {
                    group.history.forEach(entry => {
                        if (group.type === 'payable') payable += entry.amount;
                        if (group.type === 'receivable') receivable += entry.amount;
                    });
                });
                payableAmounts.push(payable);
                receivableAmounts.push(receivable);
            });

            const debtsCreditsCtx = document.getElementById('debts-credits-chart')?.getContext('2d');
            if (!debtsCreditsCtx) {
                console.error("Canvas context for debts-credits-chart not found.");
                return;
            }
            if (debtsCreditsChart) debtsCreditsChart.destroy();
            debtsCreditsChart = new Chart(debtsCreditsCtx, {
                type: 'bar',
                data: {
                    labels: names.length > 0 ? names : [translations[currentLanguage]["no_debts_data"]],
                    datasets: [
                        {
                            label: translations[currentLanguage]["payable"],
                            data: payableAmounts.length > 0 ? payableAmounts : [0],
                            backgroundColor: 'rgba(220, 53, 69, 0.8)',
                            borderColor: 'rgba(220, 53, 69, 1)',
                            borderWidth: 1
                        },
                        {
                            label: translations[currentLanguage]["receivable"],
                            data: receivableAmounts.length > 0 ? receivableAmounts : [0],
                ...
                options: {
                    ...
                    responsive: true,
                    plugins: {
                        title: { display: true, text: translations[currentLanguage]["debts_credits_chart"], color: isDarkMode ? 'white' : '#495057', font: { size: 14 } },
                        legend: { display: true, labels: { color: isDarkMode ? 'white' : '#495057' } }
                    },
                    scales: {
                        y: { beginAtZero: true, ticks: { color: isDarkMode ? 'white' : '#495057' } },
        ...
        function generateBackup() {
            const backupData = {
                ...
                appPIN: settingsPIN, permanentlyLocked: permanentlyLocked, reminders: reminders, // Add reminders to backup
                shownReminderToasts: shownReminderToasts // Add shown toasts to backup
            };
        ...
        function restoreBackup() {
            ...
            try {
                ...
                reminders = [...backupData.reminders.filter(br => !reminders.some(ar => ar.id === br.id)), ...reminders]; // Merge reminders
                shownReminderToasts = [...backupData.shownReminderToasts.filter(st => !shownReminderToasts.includes(st)), ...shownReminderToasts]; // Merge shown toasts
                ...
                checkAndDisplayReminders(); // Re-check reminders after restore
        ...
        function generateBackupFile() {
            const backupData = {
                ...
                tempCommissionData: tempCommissionData, debts: debts, expenseEntries: expenseEntries,
                savedExpenseCategories: savedExpenseCategories, isScreenLockEnabled: isScreenLockEnabled,
                appPIN: settingsPIN, permanentlyLocked: permanentlyLocked, reminders: reminders,
                shownReminderToasts: shownReminderToasts // Add shown toasts to backup
            };
        ...
        function updateTransactionType() {
            ...
            const removeButton = document.querySelector('#transaction-type + input[type="text"] + button'); // Updated selector
        ...
        function cancelEditTransaction() {
            if (isEditingTransaction) {
                // If we are in edit mode, we need to restore the transaction that was removed in editTransaction().
                // Now we add the updated one. If not editing, we just add a new one.
        ...
        function getBalanceAtEndOfDay(targetDateStr) {
            let currentEMoney = dailyOpeningBalances[targetDateStr]?.eMoney || 0;
            let currentCash = dailyOpeningBalances[targetDateStr]?.cash || 0;

            // Apply transactions on that day
            transactions.forEach(t => {
                const tDay = new Date(t.date).toISOString().split('T')[0];
                if (tDay === targetDateStr) {
        ...
        function updateTransactionTabBalances() {
            const dateFilterValue = document.getElementById('transaction-filter-date').value || new Date().toISOString().split('T')[0];
            const balances = getBalanceAtEndOfDay(dateFilterValue);
        ...
        function editTransaction(index) {
            ...
            // Temporarily remove the transaction from the array so it doesn't show in the list during editing.
                ...
                groupElement.innerHTML = `
                    ...
                    <div class="group-details">
                        ${groupData.items.map(item => {
                            ...
                            const isIn = item.action && (item.action.toLowerCase().includes('in'));
                            const amountColorClass = isIn ? 'positive' : 'negative';
                            return `
                                <div class="transaction-item">
                                    <div class="transaction-header">
                                        <span class="transaction-type">${item.type}</span>
                                        <span class="transaction-amount ${amountColorClass}">${formatNumber(item.amount)}</span>
                                    </div>
                                    <div class="transaction-details-mobile">
                                        <span class="transaction-date">${new Date(item.date).toLocaleTimeString()}</span>
        ...
        function toggleGroup(element) {
            ...
            const details = element.parentElement.querySelector('.group-details'); // Find details within the clicked header
    ...
    updateTransactionTabBalances();
        ...
        function exportToPDF() {
            ...
            const listItemsContainer = transactionListSection.querySelector('.transaction-list-items'); // Target both types of lists
        ...
        function captureForPDF(elementToCapture) {
            ...
            const clonedContent = elementToCapture.querySelector('.transaction-list-items').cloneNode(true); // Target both types of lists
        ...
        function updateDebtName() {
            const nameSelect = document.getElementById('debt-name');
            const otherInput = document.getElementById('debt-name-other');
            const removeButton = document.querySelector('#debt-name-other + button');

            otherInput.style.display = nameSelect.value === 'other' ? 'block' : 'none';
            removeButton.style.display = nameSelect.value === 'other' ? 'inline-block' : 'none';
        ...
        function removeCustomDebtName() {
            const otherInput = document.getElementById('debt-name-other');
            const customNameValue = otherInput.value.trim();

            if (customNameValue) {
                // Find and delete all debt groups with this name
                debts = debts.filter(debt => debt.name.toLowerCase() !== customNameValue.toLowerCase());
                updateExistingDebtNames();
                populateDebtNameSelect();
                saveData();
                renderDebts();
                showToast(`${translations[currentLanguage]["custom_removed"]} '${customNameValue}' ${translations[currentLanguage]["removed_from_list"]} debt names.`);
                otherInput.value = '';
            } else {
                showToast(translations[currentLanguage]["no_custom_name_to_remove"], "warning");
            }
        }

        function updateExistingDebtNames() {
            const nameSet = new Set();
            debts.forEach(debtGroup => nameSet.add(debtGroup.name));
            nameSet.add("Owner");
        ...
        function handleDebtNameInput(event) {
            ...
            const matchingNames = existingDebtNames
                .filter(existingName => existingName.toLowerCase().includes(name.toLowerCase()))
                .slice(0, 5);
        ...
        function addOrUpdateDebtEntry() {
            const nameSelect = document.getElementById('debt-name');
            const otherInput = document.getElementById('debt-name-other');
            let name = nameSelect.value === 'other' ? otherInput.value.trim() : nameSelect.value.trim();
            ...
            // Determine the type for the history entry itself (e.g., 'initial_payable' or 'initial_receivable')
            ...
            if (debtGroupIndex === -1) {
                if (!existingDebtNames.includes(name)) {
                     existingDebtNames.push(name); // Add to list of known names
                     populateDebtNameSelect(); // Update select options
                }
            ...
            saveData();
            renderDebts();
            clearDebtForm(); // This will reset editing
        ...
        function clearDebtForm() {
            document.getElementById('debt-name').value = 'other';
            document.getElementById('debt-name-other').value = ''; document.getElementById('debt-name-other').style.display = 'none';
            document.querySelector('#debt-name-other + button').style.display = 'none';
            const suggestionsContainer = document.getElementById('debt-name-suggestions');
            if (suggestionsContainer) {
        ...
        function cancelEditDebtEntry() {
            if (editingDebtEntry) {
                ...
                // We need to reconstruct the entry with its original data if the form was partially updated during cancel was pressed, so so we use form values for date/amount/note,
                ...
                // Find the original group where the entry came from.
                // If the name was changed during editing, we use the name from the form, but since we removed it based on index, and name might have changed, but in this code, we use the index stored.
                let originalGroup = debts[groupIdx]; // Use the stored index to find the original group
                if (!originalGroup) {
                    // If the group was removed (e.g., if it became empty), reconstruct it with the original name and type
                    const originalName = document.getElementById('debt-name').value.trim(); // Assume name didn't change or use form value
                    debts.push({ name: originalName, type: groupType, history: [], id: `debtGroup_${Date.now()}` });
                    originalGroup = debts[debts.length - 1];
                }

                // Reconstruct the entry with its original data from the form (as changes are discarded)
                const restoredEntry = {
                    id: id,
                    date: document.getElementById('debt-date').value,
                    amount: parseFloat(document.getElementById('debt-amount').value.replace(/,/g, '')) || 0,
                    type: type, // Keep original entry type
                    note: document.getElementById('debt-note').value.trim()
                };

                // Re-add the restored entry back to its original position
                originalGroup.history.splice(entryIdx, 0, restoredEntry);
        ...
        function renderDebts() {
            const aggregatedList = document.querySelector('#debts-credits-list .transaction-list-items');
            ...
            aggregatedList.innerHTML = ''; payableList.innerHTML = ''; receivableList.innerHTML = '';
            ...
            sortedNames.forEach(name => {
                ...
                // Net value
                const netValue = currentReceivableTotal - currentPayableTotal;
                const netAmountClass = netValue >= 0 ? 'positive' : 'negative';

                // Create aggregated item for history box
                const aggregatedDiv = document.createElement('div');
                aggregatedDiv.className = 'debt-list-item';
                aggregatedDiv.dataset.name = name;
                aggregatedDiv.innerHTML = `
                    <div class="debt-item-summary sticky-header" onclick="toggleDebtDetails(event, '${name.replace(/'/g, "\\'")}', 'aggregated')">
                        <div class="debt-name">${name}</div>
                        <span class="debt-amount ${netAmountClass}">${formatNumber(netValue)}</span>
                    </div>
                    <div class="debt-item-details-expanded" id="debt-details-${name.replace(/'/g, "\\'")}-aggregated">
                        <div class="debt-actions">
                            <button class="action-button-text delete" onclick="deleteDebtGroup('${name.replace(/'/g, "\\'")}', 'all')"><i class="fas fa-trash-alt"></i> <span data-i18n="delete_group">Delete Group</span></button>
                            <button class="action-button-text secondary" id="debt-toggle-details-button-${name.replace(/'/g, "\\'")}-aggregated" onclick="toggleDetailsButtonAction('${name.replace(/'/g, "\\'")}', 'aggregated')"><i class="fas fa-ellipsis-h"></i> <span data-i18n="details">Details</span></button>
                        </div>
                        <div class="debt-details">
                            <span class="debt-type">${translations[currentLanguage]["debts_credits_history"]}</span>
                        </div>
                        ${generateAggregatedHistoryHtml(debtAggregation.payableHistory, debtAggregation.receivableHistory)}
                    </div>
                `;
                aggregatedList.appendChild(aggregatedDiv);

                // Update totals
                totalPayable += currentPayableTotal;
                totalReceivable += currentReceivableTotal;

                // Add to separate payable list if payable > 0
                if (currentPayableTotal > 0) {
                    const payableDiv = createSummaryStyleItem(name, currentPayableTotal, 'negative', debtAggregation.payableHistory, 'payable');
                    payableList.appendChild(payableDiv);
                }

                // Add to separate receivable list if receivable > 0
                if (currentReceivableTotal > 0) {
                    const receivableDiv = createSummaryStyleItem(name, currentReceivableTotal, 'positive', debtAggregation.receivableHistory, 'receivable');
            ...
            document.getElementById('total-net-amount').textContent = formatNumber(totalReceivable - totalPayable);


            if (aggregatedList.children.length === 0) aggregatedList.innerHTML = '<p class="no-data" data-i18n="no_debts_credits_found">No debts/credits found.</p>';
        ...
        function generateAggregatedHistoryHtml(payableHistory, receivableHistory) {
            let historyHtml = '';

            if (payableHistory.length > 0) {
                historyHtml += '<h3 data-i18n="payable_debts">Payable Debts</h3>';
                historyHtml += generateHistoryHtml(payableHistory, 'payable');
            }

            if (receivableHistory.length > 0) {
                historyHtml += '<h3 data-i18n="receivable_credits">Receivable Credits</h3>';
                historyHtml += generateHistoryHtml(receivableHistory, 'receivable');
            }

            return historyHtml;
        }

        function generateHistoryHtml(history, groupType) {
        ...
        function deleteDebtGroup(name, type) {
            ...
            if (type === 'all') {
                debts = debts.filter(debt => debt.name !== name);
            } else {
        ...
        function renderDebtsFiltered(filteredDebts) {
            ...
            sortedNames.forEach(name => {
                ...
                // Add to separate payable list if payable > 0
                if (currentPayableTotal > 0) {
                    const payableDiv = createSummaryStyleItem(name, currentPayableTotal, 'negative');
                    payableList.appendChild(payableDiv);
                }

                // Add to separate receivable list if receivable > 0
                if (currentReceivableTotal > 0) {
                    const receivableDiv = createSummaryStyleItem(name, currentReceivableTotal, 'positive');
                    receivableList.appendChild(receivableDiv);
                }
            });
        ...
        function createSummaryStyleItem(name, amount, amountClass) {
            const item = document.createElement('div');
            item.className = 'summary-style-item';
            item.innerHTML = `
                <span class="summary-style-name">${name}</span>
                <span class="summary-style-amount ${amountClass}">${formatNumber(amount)}</span>
            `;
            return item;
        }
        ...
        function updatePeriodicalExpenseSummaries() {
            ...
            if (periodType === 'week') { // Assuming 'period' is a typo, use 'week' as example
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Auth state will handle the rest (loadData, render, etc.)
            // If no auth yet, loadData from localStorage as fallback
            if (!currentUser) {
                loadData();
                updateTransactionList();
                updateMoneyList();
            ...
            setInterval(checkAndDisplayReminders, 60000);
        });
        ...
        function switchTab(tabId) {
            ...
            // Hide the initial charts view if switching to a real tab
            if (tabId !== 'initial-charts-view') {
                document.getElementById('initial-charts-view').classList.remove('active');
            }

            // --- NEW LOGIC FOR AUTO SHOW/HIDE ---
            // Set default expand/collapse state for boxes in the newly active tab
            if (activeTabContent) {
                const collapsibleBoxes = activeTabContent.querySelectorAll('.transaction-list, .summary-section');

                collapsibleBoxes.forEach(box => {
                    const header = box.querySelector('h2');
                    // Check if the header is for a "Daily Summary"
                    if (header && header.getAttribute('data-i18n') === 'daily_summary') {
                        // This is a "Daily Summary" box, so EXPAND it.
                        box.classList.add('expanded');
                    } else {
                        // This is any other box (like History, Flow, etc.), so COLLAPSE it.
            ...
            // Redraw charts if their respective tabs become active
        ...
        function addReminder() {
            ...
            if (!description || !dueDate) {
                showToast(translations[currentLanguage]["reminder_fields_required"], 'error');
                return;
            }
            ...
            showToast(translations[currentLanguage]["reminder_added_success"], 'success');
        }

        function deleteReminder(id) {
            if (!confirm(translations[currentLanguage]["confirm_delete_reminder"])) return;
            reminders = reminders.filter(rem => rem.id !== id);
            saveData();
            checkAndDisplayReminders();
            showToast(translations[currentLanguage]["reminder_deleted_success"], 'success');
        }
        ...
        function checkAndDisplayReminders() {
            ...
            reminders.forEach(rem => {
                ...
                const isShown = shownReminderToasts.includes(rem.id);

                const reminderItem = document.createElement('div');
                reminderItem.className = 'reminder-item';
                if (isDue) reminderItem.classList.add('alert');

                reminderItem.innerHTML = `
                    <span>${rem.description} - <strong data-i18n="reminder_due">Due:</strong> ${new Date(rem.dueDate).toLocaleDateString()}</span>
                    <div class="reminder-actions">
                        <button class="action-button-text delete" onclick="deleteReminder('${rem.id}')"><i class="fas fa-trash-alt"></i> <span data-i18n="delete_reminder">Delete</span></button>
                    </div>
                `;

                reminderItem.addEventListener('click', (e) => {
                    if (e.target.closest('.reminder-actions')) return;
                    const allReminders = listContainer.querySelectorAll('.reminder-item');
                    allReminders.forEach(item => {
                        if (item !== reminderItem) item.classList.remove('selected');
                    });
                    reminderItem.classList.toggle('selected');
                });

                listContainer.appendChild(reminderItem);

                // Show toast for due reminders only if not shown before
                if (isDue && !isShown) {
                    showToast(`${translations[currentLanguage]["reminder_due"]}: ${rem.description}`, 'warning');
                    shownReminderToasts.push(rem.id);
                    saveData();
                }
            });

            if (reminders.length === 0) {
                listContainer.innerHTML = '<p class="no-data" data-i18n="no_reminders_found">No reminders found.</p>';
            }
        }
